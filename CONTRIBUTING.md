...
- `feature/HLAVI-50-Add-Agent-Orchestrator`
- `fix/HLAVI-72-Handle-API-Timeout`
- `docs/Update-Agent-Documentation`
- `feature/Add-Claude-Integration`
...
- `feature: add Claude API integration`
- `fix(HLAVI-72): handle API timeouts gracefully`
- `docs: update agent orchestration documentation`
- `refactor: split HTTP client logic`
...
- `[MaMu] HLAVI-50: Added Claude API Integration`
- `[MaMu] Fix API timeout handling`
- `[JoDo] Add agent orchestration documentation`
...
**hlavi-agent** depends on **hlavi-core** and is used by other Hlavi projects.
...
Projects that depend on hlavi-agent:
- **hlavi-api** - HTTP API server

### Working with Dependencies

**When hlavi-core is updated:**
...
3. Commit the updated Cargo.lock:
   ```bash
   git add Cargo.lock
   git commit -m "chore: update hlavi-core dependency"
   ```
...
git checkout -b feature/HLAVI-50-Add-Claude-Integration
```
...
```bash
git add .
git commit -m "feature(HLAVI-50): add Claude API integration with rate limiting"
```

### 3. Push and Create PR

```bash
git push origin feature/HLAVI-50-Add-Claude-Integration
```
